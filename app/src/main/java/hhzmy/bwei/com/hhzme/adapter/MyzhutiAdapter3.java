 ...
 */
public class MyzhutiAdapter3 extends RecyclerView.Adapter<MyzhutiAdapter3.MyViewHodler> {
    ...
    public MyzhutiAdapter3(Context context, Bean bean) {
        this.context = context;
        this.bean = bean;
    }

    @Override
    public MyzhutiAdapter3.MyViewHodler onCreateViewHolder(ViewGroup parent, int viewType) {
        MyViewHodler hodler = new MyViewHodler(LayoutInflater.from(context).inflate(R.layout.item_miaosha, parent, false));
        return hodler;
    }

    @Override
    public void onBindViewHolder(final MyzhutiAdapter3.MyViewHodler holder, final int position) {
        ImageLoader imageLoader = ImageLoader.getInstance();
        imageLoader.displayImage("http://image1.suning.cn/" + bean.getData().get(19).getTag().get(position).getPicUrl(), holder.miaoshaImage);
    ...
    public int getItemCount() {
        return bean.getData().get(19).getTag().size();
    }
