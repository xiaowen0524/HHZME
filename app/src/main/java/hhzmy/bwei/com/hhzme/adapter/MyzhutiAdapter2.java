 ...
 */
public class MyzhutiAdapter2 extends RecyclerView.Adapter<MyzhutiAdapter2.MyViewHodler> {
    ...
    public MyzhutiAdapter2(Context context, Bean bean) {
        this.context = context;
        this.bean = bean;
    }

    @Override
    public MyzhutiAdapter2.MyViewHodler onCreateViewHolder(ViewGroup parent, int viewType) {
        MyViewHodler hodler = new MyViewHodler(LayoutInflater.from(context).inflate(R.layout.item_miaosha, parent, false));
        return hodler;
    }

    @Override
    public void onBindViewHolder(final MyzhutiAdapter2.MyViewHodler holder, final int position) {
        ImageLoader imageLoader = ImageLoader.getInstance();
        imageLoader.displayImage("http://image1.suning.cn/" + bean.getData().get(17).getTag().get(position).getPicUrl(), holder.miaoshaImage);
    ...
    public int getItemCount() {
        return bean.getData().get(17).getTag().size();
    }
