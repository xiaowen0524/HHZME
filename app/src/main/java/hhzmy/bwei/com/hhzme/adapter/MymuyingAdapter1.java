 ...
 */
public class MymuyingAdapter1 extends RecyclerView.Adapter<MymuyingAdapter1.MyViewHodler> {
    ...
    public MymuyingAdapter1(Context context, Bean bean) {
        this.context = context;
        this.bean = bean;
    }

    @Override
    public MymuyingAdapter1.MyViewHodler onCreateViewHolder(ViewGroup parent, int viewType) {
        MyViewHodler hodler = new MyViewHodler(LayoutInflater.from(context).inflate(R.layout.item_miaosha, parent, false));
        return hodler;
    }

    @Override
    public void onBindViewHolder(final MymuyingAdapter1.MyViewHodler holder, final int position) {
        ImageLoader imageLoader = ImageLoader.getInstance();
        imageLoader.displayImage("http://image1.suning.cn/" + bean.getData().get(10).getTag().get(position).getPicUrl(), holder.miaoshaImage);
    ...
    public int getItemCount() {
        return bean.getData().get(10).getTag().size();
    }
