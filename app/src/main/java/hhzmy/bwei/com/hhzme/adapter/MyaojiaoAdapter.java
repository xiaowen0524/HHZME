 ...
 */
public class MyaojiaoAdapter extends RecyclerView.Adapter<MyaojiaoAdapter.MyViewHodler> {
    ...
    public MyaojiaoAdapter(Context context, Bean bean) {
        this.context = context;
        this.bean = bean;
    }

    @Override
    public MyaojiaoAdapter.MyViewHodler onCreateViewHolder(ViewGroup parent, int viewType) {
        MyViewHodler hodler = new MyViewHodler(LayoutInflater.from(context).inflate(R.layout.item_miaosha, parent, false));
        return hodler;
    }

    @Override
    public void onBindViewHolder(final MyaojiaoAdapter.MyViewHodler holder, final int position) {
        ImageLoader imageLoader = ImageLoader.getInstance();
        imageLoader.displayImage("http://image1.suning.cn/" + bean.getData().get(5).getTag().get(position).getPicUrl(), holder.miaoshaImage);
    ...
    public int getItemCount() {
        return bean.getData().get(5).getTag().size();
    }
