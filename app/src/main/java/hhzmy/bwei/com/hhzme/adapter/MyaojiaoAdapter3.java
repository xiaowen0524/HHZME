 ...
 */
public class MyaojiaoAdapter3 extends RecyclerView.Adapter<MyaojiaoAdapter3.MyViewHodler> {
    ...
    public MyaojiaoAdapter3(Context context, Bean bean) {
        this.context = context;
        this.bean = bean;
    }

    @Override
    public MyaojiaoAdapter3.MyViewHodler onCreateViewHolder(ViewGroup parent, int viewType) {
        MyViewHodler hodler = new MyViewHodler(LayoutInflater.from(context).inflate(R.layout.item_miaosha, parent, false));
        return hodler;
    }

    @Override
    public void onBindViewHolder(final MyaojiaoAdapter3.MyViewHodler holder, final int position) {
        ImageLoader imageLoader = ImageLoader.getInstance();
        imageLoader.displayImage("http://image1.suning.cn/" + bean.getData().get(7).getTag().get(position).getPicUrl(), holder.miaoshaImage);
    ...
    public int getItemCount() {
        return bean.getData().get(7).getTag().size();
    }
